...
/**
  ...
  */
final class IntByteBufferMap protected[debox] (ks: Array[Int], vs: Array[ByteBuffer], bs: Array[Byte], n: Int, u: Int) { lhs =>

  // map internals
  var keys: Array[Int] = ks       // slots for keys
  var vals: Array[ByteBuffer] = vs       // slots for values
  ...
  /**
    ...
    */
  final def update(key: Int, value: ByteBuffer): Unit = this.synchronized {
    @inline @tailrec
  ...
  final def apply(key: Int): ByteBuffer = {
    @inline @tailrec def loop(i: Int, perturbation: Int): ByteBuffer = {
      val j = i & mask
      val status = buckets(j)
      if (status == 0) null
...
object IntByteBufferMap {
  ...
  /**
    ...
    * performance.
    *
    * Example: Map.ofSize[Int, ByteBuffer](100).
    */
  def ofSize(n: Int): IntByteBufferMap =
  ...
  private[debox] def ofAllocatedSize(n: Int) = {
    ...
    new IntByteBufferMap(new Array(sz), new Array(sz), new Array[Byte](sz), 0, 0)
  }
